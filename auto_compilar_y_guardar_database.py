...
# 4. Instalar Ecosistema Completo de Productividad, Redes, Media, Control, Cámara, Micrófono y Conectividad LAN/Mesh VPN
print("⭐ [2/4] Instalando Ecosistema Integral de Ubuntu Core (Ofimática, Redes, VPN Mesh, Conectividad Móvil y Multimedia)...", flush=True)
subprocess.run("DEBIAN_FRONTEND=noninteractive apt-get install -y -qq libreoffice-writer libreoffice-calc libreoffice-impress vlc telegram-desktop plank papirus-icon-theme flameshot copyq evince p7zip-full unrar-free pavucontrol onboard redshift redshift-gtk qbittorrent antimicrox v4l2loopback-dkms v4l2loopback-utils pulseaudio pulseaudio-utils ffmpeg sox libportaudio2 wireguard-tools iptables bridge-utils iproute2 kdeconnect qrencode avahi-daemon iputils-ping traceroute nethogs iftop iperf3", shell=True)

print("   -> Instalando Tailscale (Mesh VPN para conectar dispositivos locales y móviles)...", flush=True)
