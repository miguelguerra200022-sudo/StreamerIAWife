...
# Definición del mando virtual Xbox 360
GAMEPAD_CAPABILITIES = {
    ...
    e.EV_ABS: [
        ...
        (e.ABS_Z, evdev.AbsInfo(value=0, min=0, max=255, fuzz=0, flat=0, resolution=0)),   # Gatillo LT
        (e.ABS_RZ, evdev.AbsInfo(value=0, min=0, max=255, fuzz=0, flat=0, resolution=0)),  # Gatillo RT
...
def get_virtual_pad():
    ...
    if virtual_pad is None:
        try:
            ...
            print("🎮 [✓] Mando Virtual Xbox 360 instanciado en el kernel (/dev/uinput)!", flush=True)
    ...
    try:
        async for message in websocket:
            try:
                ...
                # 2. Procesar Botones Estándar
                for idx, is_pressed in enumerate(buttons):
                    # Gatillo L2 (Botón 6)
                    if idx == 6:
                        pad.write(e.EV_ABS, e.ABS_Z, 255 if is_pressed else 0)
                    # Gatillo R2 (Botón 7)
                    elif idx == 7:
                        pad.write(e.EV_ABS, e.ABS_RZ, 255 if is_pressed else 0)
                    # Cruceta (D-Pad)
                    elif idx == 12: # Arriba
                        if is_pressed: pad.write(e.EV_ABS, e.ABS_HAT0Y, -1)
                        elif not buttons[13] if len(buttons) > 13 else True: pad.write(e.EV_ABS, e.ABS_HAT0Y, 0)
                    elif idx == 13: # Abajo
                        if is_pressed: pad.write(e.EV_ABS, e.ABS_HAT0Y, 1)
                        elif not buttons[12] if len(buttons) > 12 else True: pad.write(e.EV_ABS, e.ABS_HAT0Y, 0)
                    elif idx == 14: # Izquierda
                        if is_pressed: pad.write(e.EV_ABS, e.ABS_HAT0X, -1)
                        elif not buttons[15] if len(buttons) > 15 else True: pad.write(e.EV_ABS, e.ABS_HAT0X, 0)
                    elif idx == 15: # Derecha
                        if is_pressed: pad.write(e.EV_ABS, e.ABS_HAT0X, 1)
                        elif not buttons[14] if len(buttons) > 14 else True: pad.write(e.EV_ABS, e.ABS_HAT0X, 0)
                    # Botones de Acción
        ...
        print("🎮 [Plug & Play] Mando desconectado del cliente.", flush=True)
